...
function jk.process.ProcessLauncher:forFile(file, parameters)
...
function jk.process.ProcessLauncher:forCommand(command, parameters)
	...
	do
		...
		do return _g.jk.process.ProcessLauncher:forFile(file, parameters) end
	end
end
	...
	do
		...
		do
			...
			if paramCount > 0 then
				parameters = _vm:allocate_array(paramCount)
				do
					local n = 1
					while n < vsz do
						parameters[n - 1 + 1] = arr[n + 1]
						do n = n + 1 end
					end
				end
			end
			do return _g.jk.process.ProcessLauncher:forCommand(cmd, parameters) end
		end
	end
end
...
function jk.process.ProcessLauncher:addStringsToParams(parameters)
	if parameters ~= nil then
		local n = 0
		local m = _g.jk.lang.Vector:getSize(parameters)
		do
			n = 0
			while n < m do
				local s = parameters[n + 1]
				if s ~= nil then
...
function jk.process.CurrentProcess:terminate(value)
	_io:write_to_stdout("[jk.process.CurrentProcess.terminate] (CurrentProcess.sling:52:3): Not implemented" .. "\n")
end
