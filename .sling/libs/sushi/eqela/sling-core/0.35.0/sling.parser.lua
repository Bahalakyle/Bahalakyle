		...
		while true do
			...
			do
				...
				if genericTypes ~= nil then
					...
					do _g.jk.lang.Vector:append(names, v) end
				else
...
function sling.parser.SlingParser:acceptNumberLiteralExpression()
	...
	if hex ~= nil then
		...
		do
			...
			local slength = _g.jk.lang.String:getLength(st)
			if slength > 8 then
				do v:setType(_g.sling.model.IntegerDataTypeNode.TYPE_UINT64) end
				do v:setValue(_g.jk.lang.String:toLongIntegerFromHex(st)) end
			elseif slength > 4 then
				do v:setType(_g.sling.model.IntegerDataTypeNode.TYPE_UINT32) end
				do v:setValue(_g.jk.lang.String:toIntegerFromHex(st)) end
			elseif slength > 2 then
				do v:setType(_g.sling.model.IntegerDataTypeNode.TYPE_UINT16) end
				do v:setValue(_g.jk.lang.String:toIntegerFromHex(st)) end
			else
				do v:setType(_g.sling.model.IntegerDataTypeNode.TYPE_UINT8) end
				do v:setValue(_g.jk.lang.String:toIntegerFromHex(st)) end
			end
			do return v end
		end
	...
	do
		...
		if ii ~= nil then
			...
			local nc = self:peekNextCharacter()
			if nc == 117 or nc == 85 then
				do self:getNextCharacter() end
				unsigned = true
				nc = self:peekNextCharacter()
			end
			do
				local type = _g.sling.model.IntegerDataTypeNode.TYPE_ANY
				if nc == 108 or nc == 76 then
					do self:getNextCharacter() end
					if unsigned then
						type = _g.sling.model.IntegerDataTypeNode.TYPE_UINT64
					else
						type = _g.sling.model.IntegerDataTypeNode.TYPE_INT64
					end
				elseif nc == 115 or nc == 83 then
					do self:getNextCharacter() end
					if unsigned then
						type = _g.sling.model.IntegerDataTypeNode.TYPE_UINT16
					else
						type = _g.sling.model.IntegerDataTypeNode.TYPE_INT16
					end
				elseif nc == 98 or nc == 66 then
					do self:getNextCharacter() end
					if unsigned then
						type = _g.sling.model.IntegerDataTypeNode.TYPE_UINT8
					else
						type = _g.sling.model.IntegerDataTypeNode.TYPE_INT8
					end
				end
				if type == _g.sling.model.IntegerDataTypeNode.TYPE_ANY then
					if unsigned then
						type = _g.sling.model.IntegerDataTypeNode.TYPE_UINT32
					else
						type = _g.sling.model.IntegerDataTypeNode.TYPE_INT32
					end
				end
				do
											...
											do
												...
												do
													...
													do
														...
														do
															local o13 = self:acceptBinaryOperator("..=")
															if o13 ~= nil then
																local right = self:acceptConditionalExpression()
																if right ~= nil then
																	local v = _g.sling.model.StringConcatenationAssignmentExpressionNode:instance(left, right)
...
function sling.parser.SlingParser:acceptModifierKeyword(keyword)
	do return self:acceptToken1(keyword) ~= nil end
end
...
function sling.parser.SlingParser:acceptForeachLoopVariableDeclaration()
	local id = self:acceptDeclarationIdentifier()
	if not (id ~= nil) then
		do return nil end
	end
	do
		...
		do
			local v = _g.sling.model.VariableDeclarationNode:forName(id)
			do v:setSource(src) end
...
function sling.parser.SlingParser:parseForeachLoopVariableDeclaration()
	local v = self:acceptForeachLoopVariableDeclaration()
	if not (v ~= nil) then
		do self:skipSpaces() end
		do self:updateStagingToken() end
		_vm:throw_error(_g.sling.common.ReferenceError:forMessage("Expected: " .. self:getDisplayText("foreach loop variable"), self:getStagingToken(), _g.jk.lang.StackTrace:generate()))
	end
	do return v end
end
	...
	do
		...
		do
			...
			do
				local lefts = nil
				while true do
					local vdecl = self:parseForeachLoopVariableDeclaration()
					if not (lefts ~= nil) then
						lefts = {}
					end
					if self:acceptToken2(58) ~= nil then
						local vdecl2 = self:parseForeachLoopVariableDeclaration()
						local dd = _g.sling.model.KeyValueVariableDeclarationNode._construct0(_g.sling.model.KeyValueVariableDeclarationNode._create())
						do dd:setKey(vdecl, false) end
						do dd:setValue(vdecl2, false) end
						do _g.jk.lang.Vector:append(lefts, dd) end
					else
