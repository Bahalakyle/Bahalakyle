					...
					do
						...
						while n < m do
							...
							if param ~= nil then
								...
								first = false
								do sb:appendString(_g.sling.generator.SlingGenerator:dataTypeToString(_g.sling.util.ExpressionUtil:getExpressionDataType(param, nil))) end
							end
							do n = n + 1 end
						end
					end
...
function sling.generator.SlingGenerator:dataTypeToString(node)
	if not (node ~= nil) then
		_vm:throw_error(_g.sling.common.ReferenceError:forMessage("Null node for dataTypeToString", nil, _g.jk.lang.StackTrace:generate()))
	end
	...
	if (_vm:to_table_with_key(node, '_isType.sling.model.ReferenceDataTypeNode') ~= nil) then
		...
		if not (symbol ~= nil) then
			_vm:throw_error(_g.sling.common.ReferenceError:forMessage("ReferenceDataTypeNode has no symbol", rf, _g.jk.lang.StackTrace:generate()))
		end
		do
			local ss = _g.sling.generator.SlingGenerator:symbolToString(symbol, true)
			if _g.jk.lang.String:isEmpty(ss) then
				_vm:throw_error(_g.sling.common.ReferenceError:forMessage("Symbol name is an empty string!", symbol, _g.jk.lang.StackTrace:generate()))
			end
...
function sling.generator.SlingGenerator:dumpDataType(node)
	...
	do self:output(_g.sling.generator.SlingGenerator:dataTypeToString(node)) end
end
					...
					while n2 < m2 do
						...
						if child ~= nil then
							...
							do
								...
								if (_vm:to_table_with_key(child, '_isType.sling.model.MainFunctionDeclarationNode') ~= nil) then
								elseif (_vm:to_table_with_key(child, '_isType.sling.model.FunctionDeclarationBaseNode') ~= nil) then
				...
				do
					...
					if lefts ~= nil then
						...
						do
							n = 0
							while n < m do
								local lefto = _vm:to_table_with_key(lefts[n + 1], '_isType.sling.common.NodeObject')
								if lefto ~= nil then
									if not first then
										do self:output(", ") end
									end
									first = false
									if (_vm:to_table_with_key(lefto, '_isType.sling.model.VariableDeclarationNode') ~= nil) then
										do self:dumpVariableDeclaration(lefto, nil, false, nil) end
										goto _continue9
									end
									if (_vm:to_table_with_key(lefto, '_isType.sling.model.KeyValueVariableDeclarationNode') ~= nil) then
										local left = lefto
										local key = left:getKey()
										local val = left:getValue()
										if not (key ~= nil) then
											_vm:throw_error(_g.sling.common.ReferenceError:forMessage("No key variable", left, _g.jk.lang.StackTrace:generate()))
										end
										if not (val ~= nil) then
											_vm:throw_error(_g.sling.common.ReferenceError:forMessage("No value variable", left, _g.jk.lang.StackTrace:generate()))
										end
										do self:dumpVariableDeclaration(key, nil, false, nil) end
										do self:output(" : ") end
										do self:dumpVariableDeclaration(val, nil, false, nil) end
										goto _continue9
									end
									_vm:throw_error(_g.sling.common.ReferenceError:forMessage("Unsupported node type `" .. lefto:getNodeTypeName() .. "' in context `" .. "foreach loop variable" .. "'", lefto, nil))
								end
								::_continue9::
								do n = n + 1 end
							end
	...
	if (_vm:to_table_with_key(node, '_isType.sling.model.LiteralExpressionNode') ~= nil) then
		...
		if (_vm:to_table_with_key(node, '_isType.sling.model.IntegerLiteralExpressionNode') ~= nil) then
			...
			if frmt == _g.sling.model.IntegerLiteralExpressionNode.FORMAT_HEXADECIMAL then
				...
				if type == _g.sling.model.IntegerDataTypeNode.TYPE_UINT32 then
					v = _g.jk.lang.String:safeString(v) .. "U"
				elseif type == _g.sling.model.IntegerDataTypeNode.TYPE_INT8 then
					v = _g.jk.lang.String:safeString(v) .. "B"
				elseif type == _g.sling.model.IntegerDataTypeNode.TYPE_UINT8 then
					v = _g.jk.lang.String:safeString(v) .. "UB"
				elseif type == _g.sling.model.IntegerDataTypeNode.TYPE_INT16 then
					v = _g.jk.lang.String:safeString(v) .. "S"
				elseif type == _g.sling.model.IntegerDataTypeNode.TYPE_UINT16 then
					v = _g.jk.lang.String:safeString(v) .. "US"
				elseif type == _g.sling.model.IntegerDataTypeNode.TYPE_INT64 then
					v = _g.jk.lang.String:safeString(v) .. "L"
				elseif type == _g.sling.model.IntegerDataTypeNode.TYPE_UINT64 then
					v = _g.jk.lang.String:safeString(v) .. "UL"
				end
				do self:output(v) end
			end
			do return end
	...
	if (_vm:to_table_with_key(node, '_isType.sling.model.BinaryExpressionNode') ~= nil) then
		...
		if (_vm:to_table_with_key(node, '_isType.sling.model.AssignmentExpressionBaseNode') ~= nil) then
			...
			elseif (_vm:to_table_with_key(ae, '_isType.sling.model.ShiftRightAssignmentExpressionNode') ~= nil) then
				do self:output(" >>= ") end
			elseif (_vm:to_table_with_key(ae, '_isType.sling.model.StringConcatenationAssignmentExpressionNode') ~= nil) then
				do self:output(" ..= ") end
			else
