...
function jk.model.DynamicModel:addObjectToVectorField(field, value)
	local vv = self:getOrCreateFieldAsDynamicVector(field)
	if value ~= nil and (_vm:to_table_with_key(value, '_isType.jk.model.DynamicModel') ~= nil) then
		do vv:appendObject(value:toDynamicMap()) end
	else
		do vv:appendObject(value) end
	end
end
		...
		if array ~= nil then
			...
			do
				...
				while n < m do
					local ff = array[n + 1]
					if ff ~= nil then
						if _g.jk.lang.String:equals(ff:getName(), field) then
							do return true end
						end
					end
					do n = n + 1 end
