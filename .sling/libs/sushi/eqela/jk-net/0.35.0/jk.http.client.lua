		...
		do
			...
			do wc:query("POST", url, headers, bdata, function(status, headers, body)
				...
				do
					...
					local data = _vm:to_table_with_key(_g.jk.json.JSONParser:parseString(str), '_isType.jk.lang.DynamicMap')
					if not (data ~= nil) then
						...
						do rdata:mergeFrom(data) end
					end
				end
			end) end
			if rdata:getCount() < 1 then
