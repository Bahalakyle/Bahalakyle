...
function jk.server.CommonServer:getCtx()
...
function jk.server.CommonServer:initConfigValue(key, value)
	if not (key ~= nil) then
		do return end
	end
	do
	...
	if arguments ~= nil then
		...
		do
			...
			while n < m do
				...
				if arg ~= nil then
					if arg:isFlag("v") or arg:isFlag("version") then
						do self:printVersion(stdout, false) end
						do return true end
					elseif arg:isFlag("vv") then
			...
			do
				...
				while n2 < m2 do
					...
					if arg ~= nil then
						...
						elseif arg:isFlag("help") or arg:isFlag("h") then
							showHelp = true
						elseif arg:isFlag("debug") then
							self.enableDebugMessages = true
						elseif arg:isOption("listen") then
