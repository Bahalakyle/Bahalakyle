	...
	do
		...
		do
			...
			do
				...
				elseif self:getTargetPlatform() == "dotnet" then
					sb = self:createScript({
						"sling-core",
						"sling-dotnet-export"
					})
					do sb:appendString("import sling.compiler\n") end
					do sb:appendString("import sling.dotnet.compiler\n") end
					do self:startCode(ctx, sb) end
					do sb:appendString("var compiler = new SlingToDotnetCompiler(ctx)\n") end
				...
				do
					...
					if self:getBuildType() == "library" then
						if self:getTargetPlatform() == "dotnet" then
							do sb:appendString("var dotnetResult = compiler.buildToDotnetLibrary(src, null, dst, options)\n") end
							do sb:appendString("var result = dotnetResult.getLibraryBuildResult()\n") end
						else
...
function sling.tools.compiler.SlingCompilerTask:getLibraryVersions()
	if not (self.libraryVersionsValue ~= nil) then
		self.libraryVersionsValue = _g.jk.lang.DynamicMap:forObjectMap({
			["sling-core"] = "0.35.0",
			["sling-sushi-export"] = "0.18.0",
			["sling-web-export"] = "0.11.0",
			["sling-java-export"] = "0.12.0",
			["sling-dotnet-export"] = "0.3.0",
			["sling-android-export"] = "0.13.0",
			["sling-ios-export"] = "0.13.0"
		})
	end
...
function sling.tools.compiler.SlingCompilerTool:getAllPlatforms()
	...
	do _g.jk.lang.Vector:append(v, "dotnet") end
...
function sling.tools.compiler.SlingCompilerTool:forCrossPlatformLibraryDirectory()
	...
	do v:registerCommand("build", function(args)
		...
		local nargs = {}
		while args:hasMore() do
			if args:acceptOption("platform") or args:acceptOption("p") then
...
function sling.tools.compiler.SlingCompilerTool:forConsoleApplicationDirectory(mainModule)
	local v = _g.sling.tools.compiler.SlingCompilerTool._construct0(_g.sling.tools.compiler.SlingCompilerTool._create())
	do v:registerCommand("build", function(args)
		local platform = nil
		local nargs = {}
		...
		if _g.jk.lang.String:isEmpty(platform) then
			platform = "sushi"
		end
		do
			local name = "Build Sling console application to platform `" .. _g.jk.lang.String:safeString(platform) .. "'"
			local tts = _g.sling.tools.compiler.SlingCompilerTaskList:forName1(name)
			do tts:addApplication("src/" .. _g.jk.lang.String:safeString(mainModule), platform, nil) end
			do _g.sling.tools.compiler.SlingCompilerCommand:executeBuildCommand(v:getCtx(), tts, _g.jk.console.ArgumentVector:forStringVector(nargs)) end
		end
	end) end
	do return v end
...
function sling.tools.compiler.SlingCompilerTool:forDotnetApplicationDirectory(mainModule)
	do return _g.sling.tools.compiler.SlingCompilerTool:forApplicationDirectoryWithPlatform(mainModule, "dotnet", nil) end
end
